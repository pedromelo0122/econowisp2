...
function genPhoneNumberBtn($post, bool $btnBlock = false): string
{
	...
	// Siempre añadimos el link del anuncio al texto de WhatsApp
$postUrl = urlGen()->post($post);

// Si está activado "pre_filled_whatsapp_message", usamos esa plantilla; si no, usamos un texto por defecto
$baseMessage = $isPreFilledWhatsappMessageEnabled
    ? t('whatsapp_pre_filled_message', [
        'url'     => $postUrl,            // por si tu traducción lo usa
        'title'   => $post->title,
        'appName' => config('app.name'),
      ])
    : 'Estoy interesado en tu anuncio "' . $post->title . '" publicado en ' . config('app.name');

// Forzamos que el link del anuncio vaya en una línea nueva
$fullMessage = trim($baseMessage . "\r\n" . $postUrl);

// Armamos el query ?text=
$whatsAppPreFilledMessage = '?text=' . rawurlencode($fullMessage);

// Puedes mantener wa.me o usar api.whatsapp.com (mejor en desktop).
// Manteniendo wa.me:
$whatsAppLink = 'https://wa.me/' . keepOnlyNumericChars($post->phone) . $whatsAppPreFilledMessage;
// O si prefieres (recomendado):
// $whatsAppLink = 'https://api.whatsapp.com/send?phone=' . keepOnlyNumericChars($post->phone) . $whatsAppPreFilledMessage;

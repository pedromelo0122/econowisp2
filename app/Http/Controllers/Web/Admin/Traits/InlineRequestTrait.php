...
trait InlineRequestTrait
{
	...
	// tinyint, boolean, date, datetime and timestamp
	protected array $tinyintColumnTypes = ['tinyint'];
	protected array $booleanColumnTypes = ['boolean'];
	protected array $dateColumnTypes = ['date', 'datetime', 'timestamp'];
	...
	protected function updateData($model, $column): JsonResponse
	{
		...
		$allowedColumnTypes = array_merge(
			$this->tinyintColumnTypes,
			$this->booleanColumnTypes,
			$this->dateColumnTypes
		);
		...
		if (
			in_array($this->columnType, $this->tinyintColumnTypes)
			|| in_array($this->columnType, $this->booleanColumnTypes)
		) {
			$model->{$column} = (int) !((bool) $model->{$column});
		}
		if (in_array($this->columnType, $this->dateColumnTypes)) {
			$model->{$column} = (empty($model->{$column})) ? now() : null;
		}

		// Update the column data (duplicado, pero lo dejo si LaraClassifier lo necesita así)

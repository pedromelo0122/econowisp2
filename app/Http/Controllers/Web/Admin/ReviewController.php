...
class ReviewController extends PanelController
{
	public function setup()
	{
		...
		|--------------------------------------------------------------------------
		*/
		$this->xPanel->setModel(Review::class);
		$this->xPanel->with([
			'post',
			'post.user',
			'post.payment',
		...
		$this->xPanel->setRoute(urlGen()->adminUri('reviews'));
	...
	public function update(UpdateRequest $request): RedirectResponse
	{
		try {
			...
			$model = $this->xPanel->model ?? $this->xPanel->getModel();
			$dataToSave = $request->except('redirect_after_save', '_token');
			
			// Update the row in the db
			$this->xPanel->update($request->get($model->getKeyName()), $dataToSave);
			
			// Recalculate ratings for the specified product
			$reviewsId = $request->get($model->getKeyName());
			...
			$msg = (isset($e->errorInfo) && !empty($e->errorInfo[2]))
				? $e->errorInfo[2]
				: $e->getMessage();
			
			$msg = 'Error found - [' . $e->getCode() . '] : ' . $msg . '.';

...
// Delete inline: solo si existe; si no, mantenemos el botón estándar o lo quitamos
		...
		*/
		if ($this->onIndexPage) {
			...
			$isPhoneVerificationEnabled = (config('settings.sms.phone_verification') == 0);
			...
			$this->xPanel->addColumn([
			...
			
			 {
                        ...
                        $this->xPanel->addColumn([
                                ...
                                'label'         => trans('admin.Verified Seller'),
                                'type'          => 'model_function',
                                'function_name' => 'crudVerifiedSellerColumn',
                        ]);
                }
		/*
		|--------------------------------------------------------------------------
 ...
 $request = $this->handleIsAdminFromRoles($request);
                ...
                if ($request->has('is_verified_seller')) {
                        $request->request->set('is_verified_seller', $request->boolean('is_verified_seller'));
                }
